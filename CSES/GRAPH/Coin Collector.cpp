...
ll dpThis(ll node, vector<ll> &dp, vector<vector<ll>> &adjCond, vector<ll> &comp_sum, vector<ll> &roots)
{
    if(dp[node]!=-1) return dp[node];

    ll res = 0;
    for(auto &it:adjCond[node]){
        res=max(res,dpThis(it,dp,adjCond,comp_sum,roots));
    }
...
void _144()
{
    ...
    ll res = 0;
    for (ll i = 0; i < noOfScc; i++)
        res = max(res, dpThis(i, dp, adjCond, comp_sum, roots));
