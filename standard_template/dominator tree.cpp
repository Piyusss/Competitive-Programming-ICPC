        ...
        for (int p = 1; p <= log2dist; p++)
        {
            for (int n = 0; n < parents.size(); n++)
            {
                ...
                else
                {
                    ...
                    pow2mins[n][p] = min(pow2mins[n][p - 1], pow2mins[halfway][p - 1]);
                }
            }
        }
    }
...
void _144()
{
    ...
    // DSU structures for sdom calculation
    ...
    vector<pair<int, int>> sdom_times(city_num, {INT32_MAX, -1});
    for (int c : visit_order)
    {
        if (c != 0)
        {
            // Store both the visit time of sdom and the node itself
